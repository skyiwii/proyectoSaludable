...
def productos(request):
    ...
    if request.user.is_authenticated:
        favoritos = FavoritoCliente.objects.filter(id_usuario=request.user)
        favoritos_ids = [f.id_producto.id_producto for f in favoritos]
...
def distribucion(request):
    ...
    for centro in centros_distribucion:
        ...
        contactos = ContactoCentro.objects.filter(id_centro=centro)
        contactos_data = []
        for contacto in contactos:
            contactos_data.append({
                'contacto_tipo': contacto.contacto_tipo,
                'contacto_valor': contacto.contacto_valor
            })
        
        # Obtener productos disponibles en este centro
        inventarios = Inventario.objects.filter(id_centro=centro, inventario_cantidad__gt=0).select_related('id_producto')
        productos_data = []
        for inventario in inventarios:
            productos_data.append({
                'producto_nombre': inventario.id_producto.producto_nombre,
                'producto_precio': float(inventario.id_producto.producto_precio),
                'inventario_cantidad': inventario.inventario_cantidad,
                'producto_imagen': inventario.id_producto.producto_imagen.url if inventario.id_producto.producto_imagen else None
            })
...
def user_dashboard(request):
    ...
    return render(request, "verdeLimonTemplates/user_dashboard.html", context)
...
def add_to_favorites(request):
    if request.method == 'POST':
        ...
        FavoritoCliente.objects.get_or_create(id_usuario=request.user, id_producto=producto)
...
def remove_from_favorites(request):
    if request.method == 'POST':
        product_id = int(request.POST.get('product_id'))
        producto = Producto.objects.get(id=product_id)
        FavoritoCliente.objects.filter(id_usuario=request.user, id_producto=producto).delete()
        return JsonResponse({'success': True})
    return JsonResponse({'success': False, 'message': 'Método no permitido.'})
